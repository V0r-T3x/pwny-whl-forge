...
docker run -i --rm \
  ...
  kalibuild_v6:latest linux32 /bin/bash << 'EOF'
    WHEELHOUSE_PATH="/work/wheelhouse"
    # FORCE create inside the container to be sure
    mkdir -p "$WHEELHOUSE_PATH"

    # THE FIX: Force Python to identify as armv6l
    ...
    # Includes DBus fix and Atomic ops for ARMv6 linking 
    ...
    if [ ! -f "$WHEELHOUSE_PATH/numpy-${NUMPY_VERSION}-cp311-cp311-linux_armv6l.whl" ]; then
      echo "--- Phase 1: Building NumPy ${NUMPY_VERSION} ---"
      
      # 1. FORCE THE PATH: Ensure meson finds the system ninja
      export NINJA=$(which ninja || which ninja-build)
      
      # 2. Install build backend for Meson
      python3 -m pip install meson-python meson ninja Cython --extra-index-url https://www.piwheels.org/simple
      
      # 3. ARMv6 specific flags
      export CFLAGS="-march=armv6 -mfpu=vfp -mfloat-abi=hard"
      
      # 4. Run the build
      python3 -m pip wheel numpy==${NUMPY_VERSION} \
        --no-binary numpy \
        --no-build-isolation \
...
# --- Phase 2: PyTorch ---
    if ls $WHEELHOUSE_PATH/torch-2.1.*.whl 1> /dev/null 2>&1; then
      ...
      export ARCH=armv6l
      export CFLAGS="-march=armv6zk -mcpu=arm1176jzf-s -mfloat-abi=hard -mfpu=vfp"
      ...
      export USE_CUDA=0 USE_DISTRIBUTED=0 BUILD_TEST=0 USE_MKLDNN=0 \
             ...
             USE_NATIVE_ARCH=OFF INTERN_BUILD_MOBILE=0 NO_DISTRIBUTED=1 \
             USE_TBB=OFF USE_KINETO=OFF USE_FBGEMM=OFF \
             REL_WITH_DEB_INFO=0 USE_OPENBLAS=1
    ...
    python3 -m pip wheel \
      --wheel-dir=$WHEELHOUSE_PATH \
      --extra-index-url https://www.piwheels.org/simple \
      requests pillow six urllib3 idna chardet certifi

    TORCH_WHEEL=$(find "$WHEELHOUSE_PATH" -name "torch-2.1.*.whl" | head -n 1)
